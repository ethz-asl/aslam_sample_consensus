...
namespace aslam {
    ...
    bool Msac<PROBLEM_T>::computeModel(int debug_verbosity_level)
    {
        ...
        {
            fprintf(stderr,"[sm::MEstimatorSampleConsensus::computeModel] No threshold set!\n");
            return (false);
        }
        ...
        while (iterations_ < k && skipped_count < max_skip)
        {
            ...
            if (debug_verbosity_level > 1)
                fprintf(stdout,"[sm::MEstimatorSampleConsensus::computeModel] Trial %d out of %d. Best penalty is %f.\n", iterations_, static_cast<int> (ceil (k)), d_best_penalty);
            if (iterations_ > max_iterations_)
            {
                if (debug_verbosity_level > 0)
                    fprintf(stdout,"[sm::MEstimatorSampleConsensus::computeModel] MSAC reached the maximum number of trials.\n");
                break;
            }
        }
        ...
        {
            if (debug_verbosity_level > 0)
                fprintf(stdout,"[sm::MEstimatorSampleConsensus::computeModel] Unable to find a solution!\n");
            return (false);
        }
        ...
        {
            fprintf(stderr,"[sm::MEstimatorSampleConsensus::computeModel] Estimated distances (%zu) differs than the normal of indices (%zu).\n", distances.size (), indices.size ());
            return (false);
        }
        ...
        if (debug_verbosity_level > 0)
            fprintf(stdout,"[sm::MEstimatorSampleConsensus::computeModel] Model: %zu size, %d inliers.\n", model_.size (), n_inliers_count);
