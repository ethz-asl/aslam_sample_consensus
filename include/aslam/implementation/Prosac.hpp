...
namespace aslam {
    ...
    bool Prosac<PROBLEM_T>::computeModel(int debug_verbosity_level)
    {
        ...
        {
            fprintf(stderr,"[sm::ProgressiveSampleConsensus::computeModel] No threshold set!\n");
            return (false);
        }
        ...
        while (static_cast<unsigned int> (iterations_) < k_n_star)
        {
            ...
            if (selection.empty ())
            {
                fprintf(stderr,"[sm::ProgressiveSampleConsensus::computeModel] No samples could be selected!\n");
                break;
            }
            ...
            if (debug_verbosity_level > 1)
                fprintf(stdout,"[sm::ProgressiveSampleConsensus::computeModel] Trial %d out of %d: %d inliers (best is: %zu so far).\n", iterations_, k_n_star, I_N, I_N_best);
            if (iterations_ > max_iterations_)
            {
                if (debug_verbosity_level > 0)
                    fprintf(stdout,"[sm::ProgressiveSampleConsensus::computeModel] RANSAC reached the maximum number of trials.\n");
                break;
            }
        }

        if (debug_verbosity_level > 0)
            fprintf(stdout,"[sm::ProgressiveSampleConsensus::computeModel] Model: %zu size, %zu inliers.\n", model_.size (), I_N_best);
