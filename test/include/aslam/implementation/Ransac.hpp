...
namespace aslam {
    ...
    template<typename P>
    Ransac<P>::Ransac(int maxIterations, double threshold, double probability) :
    ...
    bool Ransac<PROBLEM_T>::computeModel(int debug_verbosity_level)
    {
        ...
        while (iterations_ < k && skipped_count < max_skip)
        {
            ...
            if (selection.empty ()) 
            {
                SM_ERROR ("[sm::RandomSampleConsensus::computeModel] No samples could be selected!\n");
                break;
            }
            ...
            if (debug_verbosity_level > 1)
                SM_DEBUG ("[sm::RandomSampleConsensus::computeModel] Trial %d out of %f: %d inliers (best is: %d so far).\n", iterations_, k, n_inliers_count, n_best_inliers_count);
            if (iterations_ > max_iterations_)
            {
                if (debug_verbosity_level > 0)
                    SM_DEBUG ("[sm::RandomSampleConsensus::computeModel] RANSAC reached the maximum number of trials.\n");
                break;
            }
        }

        if (debug_verbosity_level > 0)
            SM_DEBUG ("[sm::RandomSampleConsensus::computeModel] Model: %zu size, %d inliers.\n", model_.size (), n_best_inliers_count);
