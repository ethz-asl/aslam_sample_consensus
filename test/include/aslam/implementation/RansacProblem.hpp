namespace aslam {
    ...
    template<typename M>
    RansacProblem<M>::RansacProblem(bool randomSeed) : 
    ...
    template<typename M>
    RansacProblem<M>::~RansacProblem()
    {

    }
    
    template<typename M>
    bool RansacProblem<M>::isSampleGood(const std::vector<int> & sample) const
    {
    ...
    template<typename M>
    void RansacProblem<M>::drawIndexSample (std::vector<int> & sample)
      {
    ...
    template<typename M>
    boost::shared_ptr <std::vector<int> > RansacProblem<M>::getIndices () const
    {
        return indices_;
    }
    ...
    template<typename M>
    void RansacProblem<M>::getDistancesToModel (const model_t & model_coefficients, 
    ...
    template<typename M>
    void RansacProblem<M>::setUniformIndices(int N)
    {
    ...
    template<typename M>
    void RansacProblem<M>::setIndices(const std::vector<int> & indices)
    {
    ...
    template<typename M>
    int RansacProblem<M>::countWithinDistance (const model_t & model_coefficients, 
                                               const double threshold)
    {
        std::vector<double> dist;
        dist.reserve(indices_->size());
